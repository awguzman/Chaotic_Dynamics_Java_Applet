/* This is the only source file that ever needs to be modified.
 ...
 must be present in a folder for the applet to run, but Sextic.class
 is the one to be inserted in HTML code.
 The file can be renamed, but then the main class (currently Sextic)
...
public class Sextic extends Applet
 ...
 */ 
    ...
    OldR1=((Math.sqrt(Math.sqrt(r1)))*(Math.cos(theta1/5)));
    OldI1=((Math.sqrt(Math.sqrt(r1)))*(Math.sin(theta1/5)));
    k=0;
...
the computations with complex numbers on a piece of paper.*/ 
  ...
  double xSquaredRe, xSquaredIm,xCubedRe, xCubedIm, xQuarticRe,xQuarticIm,xQuinticRe, x1, xFinal, xtimesCRe, xtimesCIm, xQuinticIm, xSexticRe;
  ...
  xFinal=xSexticRe-complexMult_R(6, 0, xtimesCRe, xtimesCIm);
  return xFinal;
...
result. I put the "backup iterFunctions" for C(z-z^3/3) at the
  ...
  double xSquaredRe, xSquaredIm,xCubedIm, xCubedRe, xQuarticIm,xQuarticRe,xQuinticIm, x1, xFinal, xtimesCRe, xtimesCIm, xQuinticRe, xSexticIm;
  ...
  xtimesCRe=complexMult_R(RealC, ImagC, ReX, ImX);
  xtimesCIm=complexMult_I(RealC, ImagC, ReX, ImX);
  xFinal=xSexticIm-complexMult_I(6, 0, xtimesCRe, xtimesCIm);
  return xFinal;
