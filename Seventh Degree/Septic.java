/* This is the only source file that ever needs to be modified.
 ...
 must be present in a folder for the applet to run, but Septic.class
 is the one to be inserted in HTML code.
 The file can be renamed, but then the main class (currently Septic)
...
public class Septic extends Applet
 ...
 */ 
    ...
    OldR1=((Math.sqrt(Math.sqrt(r1)))*(Math.cos(theta1/6)));
    OldI1=((Math.sqrt(Math.sqrt(r1)))*(Math.sin(theta1/6)));
    k=0;
...
the computations with complex numbers on a piece of paper.*/ 
  ...
  double xSquaredRe, xSquaredIm,xCubedRe, xCubedIm, xQuarticRe,xQuarticIm,xQuinticRe, x1, xFinal, xtimesCRe, xtimesCIm, xQuinticIm, xSexticRe, xSexticIm, xSepticRe;
  ...
  xQuinticRe=complexMult_R(ReX, ImX, xQuarticRe, xQuarticIm);
  xQuinticIm=complexMult_I(ReX, ImX, xQuarticRe, xQuarticIm);
  xSexticRe=complexMult_R(ReX, ImX, xQuinticRe, xQuinticIm);
  xSexticIm=complexMult_I(ReX, ImX, xQuinticRe, xQuinticIm);
  xSepticRe=complexMult_R(ReX, ImX, xSexticRe, xSexticIm);
  xtimesCRe=complexMult_R(RealC, ImagC, ReX, ImX);
  xtimesCIm=complexMult_I(RealC, ImagC, ReX, ImX);
  xFinal=xSepticRe-complexMult_R(7, 0, xtimesCRe, xtimesCIm);
  return xFinal;
...
result. I put the "backup iterFunctions" for C(z-z^3/3) at the
  ...
  double xSquaredRe, xSquaredIm,xCubedIm, xCubedRe, xQuarticIm,xQuarticRe,xQuinticIm, x1, xFinal, xtimesCRe, xtimesCIm, xQuinticRe, xSexticIm, xSexticRe, xSepticIm;
  ...
  xSexticIm=complexMult_I(ReX, ImX, xQuinticRe, xQuinticIm);
  xSexticRe=complexMult_R(ReX, ImX, xQuinticRe, xQuinticIm);
  xSepticIm=complexMult_I(ReX, ImX, xSexticRe, xSexticIm);
  xtimesCRe=complexMult_R(RealC, ImagC, ReX, ImX);
  xtimesCIm=complexMult_I(RealC, ImagC, ReX, ImX);
  xFinal=xSepticIm-complexMult_I(7, 0, xtimesCRe, xtimesCIm);
  return xFinal;
